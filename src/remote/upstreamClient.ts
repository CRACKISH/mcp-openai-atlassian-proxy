...
// JSON object shape used across the upstream client
...
export class UpstreamClient {
	...
	private pending = new Map<
		string,
		{ resolve: (v: JsonObject) => void; timer: ReturnType<typeof setTimeout> }
	>();
	...
	private async connectOnce(): Promise<void> {
		...
		await this.request('initialize', {
			protocolVersion: PROTOCOL_VERSION as JsonValue,
			capabilities: {} as JsonValue,
			clientInfo: INIT_INFO as unknown as JsonValue,
		} as JsonObject);
		try {
	...
	private async openSse(): Promise<void> {
		...
		void this.consumeSse(res.body.getReader()).catch(e =>
			this.log('SSE reader error', (e as Error).message),
		);
	}

	private async waitForEndpoint(): Promise<void> {
		const start = Date.now();
		while (!this.messageTemplate && Date.now() - start < ENDPOINT_WAIT_MS) {
			await this.delay(ENDPOINT_INTERVAL_MS);
		}
	}
	...
	private dispatchJsonRpc(payload: string): void {
		try {
			...
			this.log('sse parse error', (e as Error).message);
		}
	}
	...
	private async loadTools(): Promise<void> {
		...
		this.tools = arr
			.filter(v => typeof v === 'object' && v !== null && 'name' in (v as JsonObject))
			.map(v => ({
				name: String((v as JsonObject).name),
				description:
					typeof (v as JsonObject).description === 'string'
						? this.cleanDescription(String((v as JsonObject).description))
						: undefined,
	...
	private logWhitelistedTools(): void {
		...
		for (const t of this.tools) {
			...
			try {
				...
				this.log(
					`tool ${t.name}${t.description ? ' - ' + t.description : ''} schemaKeys=${schemaKeys.join(',')}`,
				);
			} catch {
				/* ignore */
			}
	...
	private async request(method: string, params: JsonObject): Promise<JsonObject> {
		...
		const body: JsonObject = {
			...
			id,
			method,
			params: (Object.keys(params).length ? params : {}) as JsonValue,
		} as JsonObject;
	...
	private async notify(method: string, params?: JsonObject): Promise<void> {
		...
		const body: JsonObject =
			params && Object.keys(params).length
				? ({ jsonrpc: '2.0', method, params: params as JsonValue } as JsonObject)
				: ({ jsonrpc: '2.0', method } as JsonObject);
		await fetch(url, {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify(body),
	...
	private async connectLoop(): Promise<void> {
		for (;;) {
			try {
				...
				this.log(`connect failed: ${(e as { message?: string }).message || String(e)}`);
	...
	public async callTool(name: string, args: ToolArguments): Promise<ToolResponse> {
		...
		if (result && Array.isArray((result as JsonObject).content)) {
			return { content: (result as JsonObject).content as JsonValue[] };
		}
		return { content: [] };
	}
	...
	public async callToolRaw(name: string, args: ToolArguments): Promise<JsonObject> {
		return this.request('tools/call', {
			name: name as unknown as JsonValue,
	...
	private cleanDescription(desc: string): string {
		if (!desc) return '';
		// Replace control characters (Unicode category Cc) with spaces. Use
		// unicode property escapes to avoid embedding literal control ranges.
		let s = desc.replace(/\p{Cc}/gu, ' ');
		s = s
			.split(/\n\s*\n/)[0]
			.split(/\n/)[0]
			.replace(/\s+/g, ' ')
